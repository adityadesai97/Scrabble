...
class Client extends Thread{
    ...
    /* -> Buttons in the Menu */ JButton Exit; static JButton Join; 
    /* -> Messages in the Menu */ static JLabel status, title;

    Client(){
    f=new JFrame("Scrabble2");
	...
	Join = new JButton("Play!!"); Join.setActionCommand("1"); Join.addActionListener(new OptionPaneListeners()); Join.setBounds(700, 300, 300, 100);
	f.add(Exit); f.add(Join);
    ...
    private void updateUI(){
    	Client.status.setText("Lets play");
    	t.start();
        addGrid();
	    addTiles();
	    addColor();
		...
		title.setText(null);
		Join.setVisible(false); Exit.setVisible(false);
    }
    ...
    class TimerListener implements ActionListener{
        public void actionPerformed(ActionEvent e) {
            ...
            if (counter == 0){
            	...
            	timer.setText("Time is up. Player 1's turn.");
            	done.setVisible(false);
            	for(i=0;i<7;i++){
            		tile[i].setVisible(false);
            	}
...
public class Networking2 {
	 public static void main(String[] args) {
	    	...
	    	try
	        {	Client.Join.setVisible(false);
	    		Client.status.setText("Connecting to Player 1");
	            /*int port = 25000;
	            ...
	            Client.status.setText("Connected to Player 1");
